...
- How do the treatment data look like for
...
setwd("~/metropolis/Teaching/doctoral-course-longitudinal-causal-inference/")
register_data <- readRDS("practicals/_targets/objects/register_data")
str(register_data)
#+END_SRC
...
x <- protocol(x,name = "Always_bactrim",
              intervention = data.frame("bactrim" = factor(rep("1",length(intervals)-1),levels = c("0","1")),
                                        "AP" = factor(rep("0",length(intervals)-1),levels = c("0","1"))))
x <- protocol(x,name = "Always_AP",
              intervention = data.frame("bactrim" = factor(rep("0",length(intervals)-1),levels = c("0","1")),
                                        "AP" = factor(rep("1",length(intervals)-1),levels = c("0","1"))))
...
Note that we declare the variable 'AP_0' as constant because it is
exactly equal to '1-bactrim_0' and hence would confuse the analyses.

#+BEGIN_SRC R  :results output :exports (exercise-with-code) :cache yes :eval (never-plain-export)
x$names$name_constant_variables <- c("AP_0",x$names$name_constant_variables)
x <- run_rtmle(x,learner = "glmnet")
#+END_SRC

* Practical 4

Work through the second part of the R-markdown file [[./demo-rtmle-survival.Rmd]] 
The answer the following questions:

- What are the estimated risks of PCP under continuous bactrim and AP treatment, respectively?
- Consider the results for the risk of death from Practical 3 and
