...
register_data <- readRDS("~/metropolis/Teaching/doctoral-course-longitudinal-causal-inference/practicals/_targets/objects/register_data")
#+END_SRC
...
library(rtmle)
intervals <- seq(0,36,3)
x <- rtmle_init(intervals = length(intervals)-1,name_id = "id",name_outcome = "Death",name_competing = NULL,
                name_censoring = "Censored",censored_label = "censored")
register_data$timevar_data$karnofsky <- register_data$timevar_data$karnofsky[value<0,.(id,date)]
x <- add_long_data(x,
                   outcome_data=register_data$death_data,
                   censored_data=register_data$censored_data,
                   competing_data=NULL,
                   timevar_data=register_data$timevar_data)
x <- add_baseline_data(x,data=register_data$baseline_data[,start_followup_date := 0])
